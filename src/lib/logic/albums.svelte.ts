 ...
 * @returns
 */
export function validate(formData: Album): ValidationBundledErrors<AlbumPageValidationErrorType> {
  const {
    origTitle,
    bgColour,
    fgColour,
  ...
  if (tracklist.every((track) => !track.pageTitle)) {
    ...
    const tracklistValidationErrors = Array.from(
      new Set(
        tracklist.filter((l) => l.pageTitle || l.discNo || l.trackNo).flatMap((t) => t.validate()),
      ).keys(),
    ).sort((a, b) => a - b);
